...

{
    ...
    {
        $kd = $this->db->get_where('pegawai', ['email_pegawai' => $this->session->userdata('email_pegawai'), 'kd_sekolah' => $this->session->userdata('kd_sekolah')])->row_array();
        $kd_sekolah = $kd['kd_sekolah'];

        $p = $this->input->post();

        $data['tahun'] = $p['tahun'] . $p['semester'];
        $data['kd_sekolah'] = $kd_sekolah;
        $data['nik'] = $p['nik'];
